...
async function initializeMetaDirectory(projectPath) {
  for (const dir of REQUIRED_DIRS) {
    const fullPath = path.join(projectPath, dir);
    await fs.mkdir(fullPath, { recursive: true });
  }
}
async function isMetaInitialized(projectPath) {
...
async function getTrackingMode(projectPath) {
...
// src/db/init.ts
async function initializeDatabase(metaPath) {
  const dbPath = path2.join(metaPath, "k2d.db");
  const db = new Database(dbPath);
  db.exec(schema);
  const projectPath = path2.dirname(metaPath);
...
function extractSkillUsages(entry) {
  const skillUsages = [];
...
function extractMcpCalls(entry) {
  const mcpCalls = [];
  const toolCalls = extractToolCalls(entry);
  for (const tc of toolCalls) {
...
async function collectGitChanges(cwd) {
  try {
    const { stdout } = await execAsync2("git status --porcelain", { cwd });
    ...
    for (const { path: filePath, type } of statusChanges) {
      ...
      if (type !== "D") {
        try {
          const { stdout: diff } = await execAsync2(`git diff HEAD -- "${filePath}"`, { cwd });
          diffContent = diff || null;
          if (!diffContent && type === "A") {
            const { stdout: content } = await execAsync2(`git diff --cached -- "${filePath}"`, { cwd });
      ...
      let commitHash = null;
      try {
        const { stdout: hash } = await execAsync2("git rev-parse HEAD", { cwd });
...
function snapshotDiffToChanges(diff, oldSnap, newSnap) {
  const changes = [];
...
async function listFiles(dirPath, pattern) {
  try {
    ...
    return files.map((f) => path5.basename(f, path5.extname(f)));
  } catch {
    return [];
  }
}
async function collectClaudeConfig(projectPath) {
  ...
  const [settingsJson, settingsLocalJson, claudeMdRoot, claudeMdClaude, mcpJson, lspJson] = await Promise.all([
    safeReadFile(path5.join(claudeDir, "settings.json")),
    safeReadFile(path5.join(claudeDir, "settings.local.json")),
    safeReadFile(path5.join(projectPath, "CLAUDE.md")),
    safeReadFile(path5.join(claudeDir, "CLAUDE.md")),
    safeReadFile(path5.join(claudeDir, ".mcp.json")),
    safeReadFile(path5.join(claudeDir, "lsp.json"))
  ]);
  const [agentsList, skillsList, pluginsList, rulesList, commandsList] = await Promise.all([
    listFiles(path5.join(claudeDir, "agents"), "*.md"),
    listSkills(claudeDir),
    listPlugins(claudeDir),
    listFiles(path5.join(claudeDir, "rules"), "*.md"),
    listFiles(path5.join(claudeDir, "commands"), "*.md")
  ]);
  return {
...
// src/extractors/skill-inference.ts
var SKILL_KEYWORDS = {
...
function inferSkillIntroductionReason(context, skillName) {
...
// src/extractors/phase-inference.ts
var PHASE_SIGNALS = {
  init: {
...
function inferPhaseFromSkills(skillNames) {
...
function inferPhaseFromFiles(filePaths) {
  const phaseScores = {
    init: 0,
    requirements: 0,
    design: 0,
...
function inferPhaseFromContext(context) {
  const contextLower = context.toLowerCase();
  const phaseScores = {
    init: 0,
    requirements: 0,
...
function inferProjectPhase(params) {
  const phases = [];
...
function saveToolCalls(db, turnId, toolCalls) {
  const stmt = db.prepare(`
    INSERT INTO tool_calls (turn_id, tool_name, tool_type, parameters, result_status, result_summary, execution_time_ms)
    VALUES (?, ?, ?, ?, ?, ?, ?)
  `);
  for (const tc of toolCalls) {
    stmt.run(turnId, tc.tool_name, tc.tool_type, JSON.stringify(tc.parameters), tc.result_status, tc.result_summary, tc.execution_time_ms);
  }
}
function saveFileChanges(db, turnId, changes) {
  const stmt = db.prepare(`
    INSERT INTO file_changes (turn_id, file_path, change_type, diff_content, commit_hash, old_hash, new_hash)
    VALUES (?, ?, ?, ?, ?, ?, ?)
  `);
  for (const change of changes) {
    stmt.run(turnId, change.file_path, change.change_type, change.diff_content, change.commit_hash, change.old_hash, change.new_hash);
  }
}
function saveSkillUsages(db, turnId, usages) {
  const stmt = db.prepare(`
...
function saveMcpCalls(db, turnId, calls) {
  const stmt = db.prepare(`
...
function saveConfigSnapshot(db, turnId, config) {
  db.prepare(`
  ...
  `).run(turnId, config.settings_json, config.settings_local_json, config.claude_md, config.mcp_json, config.lsp_json, JSON.stringify(config.agents_list), JSON.stringify(config.skills_list), JSON.stringify(config.plugins_list), JSON.stringify(config.rules_list), JSON.stringify(config.commands_list));
}
function saveConfigChanges(db, turnId, changes) {
  const stmt = db.prepare(`
  ...
  `);
  for (const change of changes) {
    stmt.run(turnId, change.config_type, change.item_name, change.change_type, change.before_value, change.after_value, change.diff_summary);
  }
}
function updateSkillLifecycle(db, skillName, turnId, reason) {
...
function recordPhaseTransition(db, phaseName, turnId, dominantSkills, dominantTools) {
  db.prepare(`
  ...
  `).run(phaseName, new Date().toISOString(), turnId, JSON.stringify(dominantSkills), JSON.stringify(dominantTools));
}
function getProjectPhases(db) {
  return db.prepare("SELECT * FROM project_phases ORDER BY started_at").all();
}
function saveCompleteTurnData(db, data) {
  const turnId = createTurn(db, data.sessionId, data.turnNumber, data.turnData.user_message, data.turnData.assistant_response);
...
async function main() {
  try {
    ...
    if (input.stop_hook_active) {
      const output2 = { skipped: true, reason: "stop_hook_active" };
      console.log(JSON.stringify(output2));
      return;
    }
    const projectPath = process.cwd();
    const metaPath = path6.join(projectPath, "meta");
    ...
    if (!db) {
      ...
      const trackingMode2 = isGit ? "git" : "snapshot";
    ...
    const output = { skipped: true, reason: String(error) };
    console.log(JSON.stringify(output));
  }
}
main();
